...
void PlanFFT<compType>::initialize(Field<compType>*  rfield,Field<compType>*  kfield,const int mem_type )
{
  ...
  {
    std::cerr<<"Latfield2d::PlanFFT::initialize : fft curently work only for fields with same number of components"
        <<std::endl;
    std::cerr<<"Latfield2d::PlanFFT::initialize : coordinate and fourier space fields have not the same number of components"
        <<std::endl;
    std::cerr<<"Latfield2d : Abort Process Requested"
        <<std::endl;
  ...
  tempMemory.setTemp((long)(rSize_[0]+2)  * (long)(rSizeLocal_[1]+2) * (long)(rSizeLocal_[2]+2));
  	...
  	if(rfield->lattice().dim()!=3)
  	{
  		if(parallel.isRoot())
  		{
  			std::cerr<<"Latfield2d::PlanFFT::initialize : fft curently work only for 3d cubic lattice"
                <<std::endl;
  			std::cerr<<"Latfield2d::PlanFFT::initialize : real lattice have not 3 dimensions"
                <<std::endl;
  			std::cerr<<"Latfield2d : Abort Process Requested"
                <<std::endl;
  	...
  	if(rSize_[0]!=rSize_[1] | rSize_[1]!=rSize_[2])
  	{
  		if(parallel.isRoot())
  		{
  			std::cerr<<"Latfield2d::PlanFFT::initialize : fft curently work only for 3d cubic lattice"
                <<std::endl;
  			std::cerr<<"Latfield2d::PlanFFT::initialize : real lattice is not cubic"
                <<std::endl;
  			std::cerr<<"Latfield2d : Abort Process Requested"
            <<std::endl;
...
void PlanFFT<compType>::initialize(Field<double>*  rfield,Field<compType>*   kfield,const int mem_type )
{
  ...
  if(rSize_[0]!=rSize_[1] or rSize_[1]!=rSize_[2])
  {
    if(parallel.isRoot())
    {
      std::cerr<<"Latfield2d::PlanFFT::initialize : fft curently work only for 3d cubic lattice"<<std::endl;
      std::cerr<<"Latfield2d::PlanFFT::initialize : real lattice is not cubic"
        <<std::endl;
      std::cerr<<"Latfield2d : Abort Process Requested"
        <<std::endl;
