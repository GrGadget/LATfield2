...
namespace LATfield2
{
   ...
   array_size,std::string  filename_str, std::string dataset_name_str);
    ...
    int load_hdf5_externC(
        ...
        const int *size,
        const int *sizeLocal,
        int halo, int lat_dim,std::string  filename_str, std::string
    dataset_name_str);

template<class fieldType>
int save_hdf5(fieldType *data,hid_t type_id,int array_size,
    const long file_offset[2],
    const int *size,
    const int * sizeLocal,int halo, int lat_dim,int
...
template<class fieldType>
int load_hdf5(fieldType *data,
    const long file_offset[2],
    const int *size,
    const int * sizeLocal,
    int halo, int lat_dim,int comp,std::string  filename_str, std::string dataset_name_str)
{
